...
<form action="clinicianProductSummary" method="post">
	 Select Customer :
...
<?php if($_POST){?>
	...
	if($clinician){
	    ...
	    ->select('p.name, p.hn, sum(amount_total) as total,count(amount_total)')
		...
		$prange = Yii::app()->db->createCommand()
	    ->select('p.display_name as name, count(*) as count, avg(oi.order_per_day) as avg ')
	    ->from('orders as o , order_items as oi , products as p')
	    ->where('p.id=oi.product_id and o.id = oi.order_id and o.request_received >= :from_date and o.request_received <= :to_date and o.customer_id = :cid and o.clinician_id=:clinician_id' ,array(':clinician_id'=>$clinician['id'],':cid'=>$customer['id'],':from_date'=>$from_date,':to_date'=>$to_date))
		->group('p.id')
		->order('count desc')
	    ->queryAll();
	...
	<table style="width:500px;" border="1"><tr><th>Product</th><th>Order Count</th><th>Order AVG</th></tr>
	<?php  foreach ($prange as $r) { 
	
		echo "<tr><td>".$r['name']."</td><td>".$r['count']."</td><td >".number_format($r['avg'],2)."</td></tr>";
